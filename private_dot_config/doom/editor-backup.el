...
(defun tetov/org-file-has-any-tag-p (filename &rest tags)
...
(setq-default tetov/editor-backup-dir (file-name-concat "editor-backups" "emacs" (system-name)))
...
(defun tetov/backup-each-save-filter (filename)
  ...
  (let* ((filename-ignore-patterns `(,tetov/editor-backup-dir
         ...
         (should-ignore (or (seq-some (lambda (pattern)
                                        (string-match-p pattern filename))
                                      filename-ignore-patterns)
                            (and (string-match-p "\\.org$" filename)
